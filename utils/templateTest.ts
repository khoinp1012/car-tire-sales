...
export const testTemplateLoading = async (): Promise<void> => {
  Logger.log('🧪 Starting template loading test...');


  try {
    // Initialize templates
    Logger.log('📋 Step 1: Initializing templates...');

    await initializeTemplates();

    // Test direct template loading
    Logger.log('📋 Step 2: Testing direct template loading...');

    const directTemplate = await loadTemplateFromAssets('invoice_template.html');
    Logger.log(`✅ Direct loading successful: ${directTemplate.length} characters`);


    // Test template utility loading
    Logger.log('📋 Step 3: Testing template utility loading...');

    const utilityTemplate = await loadHTMLTemplate('default');
    Logger.log(`✅ Utility loading successful: ${utilityTemplate.length} characters`);
    ...
    for (const templateType of templateTypes) {
      try {
        Logger.log(`📋 Step 4.${templateTypes.indexOf(templateType) + 1}: Testing ${templateType} template...`);

        const template = await loadHTMLTemplate(templateType);
        Logger.log(`✅ ${templateType} template loaded: ${template.length} characters`);

      } catch (error) {
        Logger.error(`❌ Failed to load ${templateType} template:`, error);

      }
    }

    Logger.log('🎉 Template loading test completed successfully!');


  } catch (error) {
    Logger.error('❌ Template loading test failed:', error);
...
export const quickTemplateTest = async (): Promise<boolean> => {
  try {
    ...
    Logger.error('Quick template test failed:', error);
