...
export default function InventoryForm({ mode = 'insert', itemData, documentId, onSuccess }: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === 'insert') {
        docData = await inventoryService.createItem(commonPayload);
      } else if (mode === 'modify' && documentId) {
        docData = await inventoryService.updateItem(documentId, commonPayload);
      }

      const sequence = docData?.sequence;

      setShowSuccess(true);
      setTimeout(() => setShowSuccess(false), 2000);

      if (onSuccess) onSuccess(docData);

      // Optionally print after insert/modify
      // Note: sequence might be 0 if offline, until it syncs
      if (enablePrint && sequence !== undefined) {
        await handlePrintInventory(sequence);
      }
