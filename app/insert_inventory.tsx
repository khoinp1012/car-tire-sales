...
import appwrite, { account, DATABASE_ID, INVENTORY_COLLECTION_ID, AUTOFILL_COLLECTION_ID } from '@/constants/appwrite';
...
function InsertInventoryContent() {
  ...
  const handlePrintTest = async () => {
    ...
    if (!selectedPrinter) {
      ...
      Alert.alert(i18n.t('printTest', { locale: lang }), i18n.t('printTestCompletedCheckConsole', { locale: lang }));
      return;
    }
  ...
  const handlePrintInventory = async (sequence: number) => {
    ...
    // Console log what would be printed (for testing without printer)
    const qrValue = `${QR_PREFIXES.INVENTORY}${sequence}`;
  ...
  const handlePrintInventoryWithData = async (printData: any) => {
    ...
    // Console log what would be printed (for testing without printer)
    const qrValue = `${QR_PREFIXES.INVENTORY}${printData.sequence}`;
    console.log('=== PRINT INVENTORY OUTPUT ===');
    console.log(`QR Code Data: "${qrValue}"`);
    console.log('QR Code Size: 200x200');
  ...
  const handleSubmit = async () => {
    ...
    for (let i = 0; i < qty; i++) {
      try {
        ...
        if (doc.$sequence !== undefined) {
          ...
          await databases.updateDocument(DATABASE_ID, INVENTORY_COLLECTION_ID, doc.$id, { sequence: parsedSequence });
    ...
    if (successCount > 0) {
      ...
      const insertAutofillIfNotExist = async (field_name: string, field_value: string) => {
        ...
        try {
          const result = await databases.listDocuments(DATABASE_ID, AUTOFILL_COLLECTION_ID, [
          ...
          if (!result.documents || result.documents.length === 0) {
            const created = await databases.createDocument(DATABASE_ID, AUTOFILL_COLLECTION_ID, ID.unique(), {
              field_name,
    ...
    if (errorCount > 0) {
      Alert.alert(i18n.t('error', { locale: lang }), i18n.t('errorOccurredWithCount', { locale: lang, count: errorCount }));
    }
  };
