...
export default function ScanModifyInventoryScreen() {
  ...
  useEffect(() => {
    ...
    if (isEmulator) {
      console.log('[QR SCAN INVENTORY] Running in emulator, showing manual input');
    ...
    (async () => {
      try {
        console.log('[QR SCAN INVENTORY] Requesting camera permissions...');
        const { status } = await Camera.requestCameraPermissionsAsync();
        console.log('[QR SCAN INVENTORY] Camera permission status:', status);
        setHasPermission(status === 'granted');
        if (status !== 'granted') {
          console.log('[QR SCAN INVENTORY] Camera permission denied, showing manual input');
          setShowManualInput(true);
          Alert.alert(
        ...
        console.error('[QR SCAN INVENTORY] Error requesting camera permission:', e);
        setHasPermission(false);
        setShowManualInput(true);
      }
    })();
  ...
  const handleBarCodeScanned = ({ data }: { data: string }) => {
    console.log('[QR SCAN INVENTORY] Scanned data:', data);
    setScanned(true);
    if (!data || data.trim() === '') {
      console.log('[QR SCAN INVENTORY] Invalid/empty QR code data');
      Alert.alert('Invalid QR code', 'Please try scanning again');
      setTimeout(() => setScanned(false), 2000);
      return;
    }
    console.log('[QR SCAN INVENTORY] Navigating to modify_inventory with data:', data);
    router.push({ pathname: '/modify_inventory', params: { scanned: data.trim() } });
  };

  const onCameraReady = () => {
    console.log('[QR SCAN INVENTORY] Camera is ready');
    setCameraReady(true);
  };
      ...
      <View style={styles.center}>
        ...
        <ThemedButton
          ...
          onPress={() => {
            ...
            console.log('[QR SCAN INVENTORY] Manual input submitted:', manualInput.trim());
            router.push({ pathname: '/modify_inventory', params: { scanned: manualInput.trim() } });
          }}
          color="#1976d2"
        />
      </View>
    ...
    <View style={styles.container}>
        ...
        <>
          ...
          {cameraReady && !scanned && (
            <Text style={styles.instructions}>
              Point camera at QR code to scan inventory
            </Text>
          )}
        </>
      )}
      ...
      {scanned && !isEmulator && (
        <ThemedButton 
          ...
          onPress={() => {
            console.log('[QR SCAN INVENTORY] Resetting for new scan');
            setScanned(false);
        ...
        <View style={styles.overlay}>
          ...
          <ThemedButton
            ...
            onPress={() => {
              ...
              console.log('[QR SCAN INVENTORY] Manual overlay input submitted:', manualInput.trim());
              router.push({ pathname: '/modify_inventory', params: { scanned: manualInput.trim() } });
            }}
            color="#1976d2"
          />
